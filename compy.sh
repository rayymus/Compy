...
cmd_install() {
    ...
    echo ""
    echo "0. Install system dependencies:"
    echo "   brew install ripgrep                         # required"
    echo "   brew install ollama whisper-cpp ffmpeg       # recommended"
    echo ""
    echo "1. Install Python dependencies:"
    echo "   pip install -r requirements.txt"
    echo ""
    echo "2. Pull the Ollama model (for LLM-ranked results):"
    echo "   ollama pull qwen2.5-coder:1.5b"
    echo ""
    echo "3. Install the companion extension:"
    echo "   npm install --prefix $ROOT/compy/extension"
    echo "   npm run build --prefix $ROOT/compy/extension"
    echo "   (then load compy/extension/ in VS Code: 'Developer: Install Extension from Location...')"
    echo ""
    echo "4. Start Ollama:"
    echo "   ./compy.sh ollama-start"
    echo ""
    echo "5. Run the overlay:"
    echo "   ./compy.sh overlay"
    echo ""
    echo "6. Or, add Compy to your login items for auto-start:"
